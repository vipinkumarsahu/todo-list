...
(function($) {
  ...
  $('[data-chat-input]').on('keypress',function(e){
    ...
    if(e.which == 13) {
        if($sendBtn.val() != ''){
            var el = $sendBtn.attr('data-chat-conversation');
            $(el).append('<div class="message clearfix">'+
             '<div class="chat-bubble from-me">'+$sendBtn.val()+
             '</div></div>');
            var msgObj = {
                message: $sendBtn.val(),
                f_id: $sendBtn.attr('data-friendid')
            }
            sendConversation(msgObj);//Save Message on Server
            $sendBtn.val('');
...
(function($) {
    ...
    var MobileView = function(element, options) {
        ...
        self.element.unbind('click').on('click', function(e) {
            e.preventDefault();
            if(self.element.parent().hasClass('new-chat-highlight')){
                self.element.parent().removeClass('new-chat-highlight')
            }
            ...
            self.options.onNavigate(toView, data.viewAnimation,this);
            return false;
        })
        return this; // enable chaining
    };
    ...
    $.fn.pgMobileViews.defaults = {
        ...
        onNavigate: function(view, animation,bthis) {
            if($(bthis).attr('data-get-conv') == '1'){
                getConversation(view,animation,bthis);
            }
        }
    }
...
(function($) {
    ...
    $(document).on('click.pg.quickview.data-api touchstart', '[data-toggle="quickview"]', function(e) {
        ...
        $('#chat-user-list').html('');
        $('#chat-user-list').addClass('circle-loader');                
        var elem = $(this).attr('data-toggle-element');
        var message = $(this).attr('data-message');
        var userid = $(this).attr('data-user-id');
        var senderArr = $(this).attr('msg-senders');
        $(this).removeAttr('msg-senders');
        if (Modernizr.csstransitions) {
            if(message){
                var data = {
                    id: userid
                };
                getElemView(data, '/user/get-user-friends', '#chat-user-list', '/chat_user_list',function(data){
                    $('#chat-user-list').removeClass('circle-loader');
                    var data1 = data.data.data;
                    for (var i = 0; i < data1.length; i++) {
                        if(senderArr != "" && senderArr != undefined && senderArr.indexOf(data1[i].user_id) != -1){
                            $('#chat-user-list').find('#user-'+data1[i].user_id).addClass('new-chat-highlight');
                        }
                    }
                },'html');
            }
